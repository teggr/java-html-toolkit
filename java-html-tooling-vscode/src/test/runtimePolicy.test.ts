...
test('defaults to maven-first strategy when unset or unknown', () => {
    assert.equal(normalizeBuildStrategy(undefined), 'maven-first');
    assert.equal(normalizeBuildStrategy('unknown'), 'maven-first');
});
