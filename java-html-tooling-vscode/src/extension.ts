...
async function runPreview(context: vscode.ExtensionContext, args: PreviewCommandArgs): Promise<void> {
    const { document, className, methodName, previewLabel } = args;

    const projectRoot = findMavenRoot(document.uri.fsPath);
    if (!projectRoot) {
        vscode.window.showErrorMessage(
            'Java HTML Tooling: Cannot find a Maven project root (pom.xml) for this file.',
        );
        return;
    }
    ...
    const displayName = previewLabel?.trim() || methodName;
    const previewName = `${displayName} – Java HTML Preview`;
...
async function showDiagnostics(): Promise<void> {
    ...
    const previewItems = [...activePreviews.values()]
        .map((preview) => {
            ...
            const labelSuffix = preview.previewLabel ? ` [${preview.previewLabel}]` : '';
            return `- ${preview.className}#${preview.methodName}${labelSuffix} (${relativePath})`;
        })
        .join('\n');
...
function runJavaMethod(
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const argFileContent = `-cp\n${classpath}\ndev.rebelcraft.html.preview.PreviewRunner\n${className}\n${methodName}`;
...
<head>
    <meta charset="UTF-8">
    <title>Java HTML Preview – Error</title>
    ${cssContent}
</head>
<body>
