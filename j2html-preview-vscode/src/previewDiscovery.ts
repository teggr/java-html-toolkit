...
export function discoverPreviewTargets(
    ...
    const annotationLines = collectPreviewAnnotationLines(lines);
    ...
    for (const annotationLine of annotationLines) {
        const methodCandidate = methodCandidates.find(
            (candidate) =>
                candidate.selectionStartLine >= annotationLine
                && candidate.selectionStartLine - annotationLine <= MAX_ANNOTATION_DISTANCE,
        );
